...
\begin{Lemma}
	\[P(\Z) \backslash \SL_2(\Z) \cong \{(m,n)\in \Z^2\backslash\{(0,0)\} : m,n \text{ are co-prime}\}\]
\end{Lemma}
\proofbar{
	For brevity denote \(\{(m,n)\in \Z^2\backslash\{(0,0)\} : m,n \text{ are co-prime}\}\) by \(\Z^2_{\mathrm{gcd}=1}\).
	Then if \((c,d) \in \Z^2_{\mathrm{gcd}=1}\) by Bezout's lemma there exists \(a,b\in \Z\) such that 
	\[ad - bc = \mathrm{gcd}(c,d) = 1.\]
	Therefore \(\begin{pmatrix}
		a & b \\ c& d
	\end{pmatrix}\in \SL_2(\Z)\). This suggests a map 
	\[\Z^2_{\mathrm{gcd}=1} \to \SL_2(\Z) ,\]
	by sending a pair \((c,d)\) to a matrix whose second row is \((c,d)\) and whose top row is a pair of Bezout coefficients as above. To make this a well defined map we can either \textit{choose} Bezout coefficient for each pair, or \textit{quotient} by some subgroup such that all Bezout coefficient choices end up in the same coset. 
	So now consider the map
	\[\Z^2_{\mathrm{gcd}=1} \to \SL_2(\Z) \xrightarrow{\mathrm{proj}} P(\Z) \backslash \SL_2(\Z).\]
	
	First we need to check that the image in the projection is independent of the choice of Bezout coefficient, to see that this map is well defined.
	
	Now check the map is injective.
	
	Now check the map is surjective.
