...
If \(P = MU\) and \(P' = M'U'\) are two standard parabolics of \(G\) that are conjugate, i.e. such that for \(w\in G(k)\) we have \(wMw\inv = M'\)\todo{does this imply that P and P
 anre conjugate? des this even matter?}.
...
\end{cases},\]
 ...
 \proofbar{
 	...
 	Using the product formula, i.e. for all \(x\in \Q^\times\) we have that \(\prod_\nu |x|_\nu = 1\), we get that \(\phi_s\) is left \(\SL_2(\Q)\) invariant\todo[inline]{apparently this is wrong? I think this is just what garret says though.}. \(Z(\mathfrak{g})\) finiteness can be checked using the known Casimir of the Lie algebra of \(\SL_2(\R)\), which we again omit.
 }
...
\todo{E4 eisenstein series things? Chenyan wants me to add the matrix comps above, but eariler she said its too trivial?}
