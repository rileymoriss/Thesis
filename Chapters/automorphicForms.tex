...
\section{Modular Forms} \label{sec:modular-forms}
	...
	\end{Lemma}
	\proofbar{
		Unraveling the definitions we require two constants \(r,c\in \R^+\) such that for all \(g = \begin{pmatrix}
		a & b \\ c & d
		...
		\todo[inline,color=blue]{Modular forms moderate growth}
        but t
        
        The growth condition can be precisely stated as the \(\lim_{x\to \infty}f(xi)\) being bounded. Hence 
	}
	...
	\proofbar{ Only a sketch. 
		...
		We have the coordinates on \(\begin{pmatrix}y^{1/2} & x y^{-1/2}\\ & y^{-1/2}\end{pmatrix}SO_2(\R)=\SL_2(\R)\) from \cite[1.19]{bumpAutomorphicFormsRepresentations1997} in which 
