...
Following \cite[as above..]{borelAutomorphicFormsRepresentations1979} \todo[inline]{fix}.
...
\todo[inline]{should I make clear what a subquotient is of a g,K module..?}
...
Recall that an automorphic form \(\phi\in \mathcal{A}(U(\A)M(F) \backslash G(\A))\) is called cuspidal  if all its constant terms vanish, see the next section for more detail.\todo[inline]{reference the definition in the next chapter}
...
\end{Remark}
\todo[inline]{what is meant though by all the subquotient stuff in this case though....}
...
There is a conjecture by Moeglin and Waldspurger that this is a superflous conventions \todo[inline]{reference Chenyan?}

However Yamana has the following statement 
\todo[inline]{Chengjing example of isotypic subspaces}
...
Here we will let \(\pi, \pi'\) be two automorphic representations of \(G(\A)\) though of as representations of \(\mathcal{H}(G(\A))\). Because the Hecke algbra is commutative \todo[inline]{reference} we can define a \(G(\A)\) module structure on their tensor product as follows
