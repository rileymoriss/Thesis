...
\proofbar{
	...
	is \(K_\infty\) equivariant by the definition of the adjoint action. Now if \(\phi \in \mathcal{A}(\Gamma \backslash G(F_\infty))\) then its span is a finite dimensional \(K_\infty\) module which we will denote \(M_\phi\). Then \(k.X\phi\) is in the image of \(\mathfrak{g}\tensor M_\phi\) under the map \ref{eq:1}. But the Lie algebra is finite dimensional and \(M_\phi\) is finite dimensional so this image is finite dimensional. Therefore the \(K_\infty\) span of \(X\phi\) is finite dimensional and so \(X\phi\) is \(K_\infty\)-finite.
}
