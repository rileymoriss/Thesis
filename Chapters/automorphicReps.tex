...
Now we define a \((\mathfrak{g}, K_\infty)\)-module structure on the space of automorphic forms. Recall that by definition we have that 
\[\mathcal{A}(U(\A)M(F)\backslash G(\A)) \subseteq C^\infty(G(\A_f)) \tensor C^\infty(G(F_\infty)).\]
If \(\phi_f\tensor \phi_\infty \in \mathcal{A}(U(\A)M(F)\backslash G(\A)) \) then \(K_\infty\subseteq G(F_\infty)\) a maximal compact subgroup in good position acts by 
...
&\defeq \{\phi \in \mathcal{A}_0(U(\A)M(F) \backslash G(\A)) : \forall k\in K, \; \phi_k\in\mathcal{A}(M(k) \backslash M(\A))_{\textcolor{red}{\pi}} \}
\end{align*}
