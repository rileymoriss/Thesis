...
Note that this quotient may not be a group, because H is not required to be normal. The use of this lemma is called \textbf{unfolding} the integral.
    ...
    \proofbar{
        ...
        Analysing the summands, by the proof of the second isomorphism theorem for groups we have a bijection
        \[P(F)\setminus P(F)w\inv P'(F) \cong P(F)\cap w\inv P'(F) \setminus w\inv P'(F). \]
        Note that the group in the denominator still acts on \(w\inv P'(F)\) \textit{on the left}.
        Multiplication by \(w\) is a bijection that sends for \(p'\in P'(F)\)
        \[[w\inv p'] = \{pw\inv p' : p\in P(F)\cap w\inv P'(F) \} \in P(F)\cap w\inv P'(F) \setminus w\inv P'(F) ,\]
        to 
        \[\{wpw\inv p' : p\in P(F)\cap w\inv P'(F) \} \in wP(F)w\inv \cap P'(F) \backslash P'(F).\]
...
\end{Lemma}
%\todo{need to make the root datum weyl group stuff explicit and then describe these representatives (minimaal length ones)}
...
\end{Lemma}
    ...
    \proofbar{
        ...
        First let \(m' = wp_1w\inv\) and \(u' = (m')\inv wp_2w\inv m'\) for \(p_1, p_2 \in P(F)\) then 
        \begin{equation*}
            \begin{aligned}
                m'u' &= m'(m')\inv wp_2w\inv wp_1w\inv \\
                     &= wp_2p_1w\inv \in wP(F)w\inv.\\
            \end{aligned}
        \end{equation*}
    }
...
\end{comment}
    ...
    \begin{Lemma}[\cite{moeglinSpectralDecompositionEisenstein1995} II.1.7 (6)] For \(w\in R\) as above
