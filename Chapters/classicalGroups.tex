...
\begin{example}
    ...
    Let our group be defined over the global field \(k\)\todo[inline]{fix a global convention}. We will often need to fix a maximal compact subgroup \(K\subseteq G(\A)\). These are not unique and as such when fixing one it can be arranged to have many nice properties \cite[I.1.4]{moeglinSpectralDecompositionEisenstein1995}. In particular if we have a group \(G\) and a fixed Borel \(B\):
    \begin{itemize}
