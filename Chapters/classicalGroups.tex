...
Parabolic subgroups have two equivalent formulations, both useful.\todo[inline]{Do I need to give a reference for definitions?}
\begin{definition}
...
\begin{example}
    ...
    \end{example}
     ...
     Perhaps a more compelling reason to call these Tori is that they play the same role in the classification as the genuine tori, \(\mathbb{T}^2\), in the theory of Lie groups. \todo[inline]{example maybe idk} A subgroup that is isomorphic to a split torus and is maximal in this respect is called a maximal split torus. 
     \begin{example}
    ...
    \todo[inline]{put a proof of something being a Borel}
    ...
    Let our group be defined over the global field \(k\)\todo[inline]{fix a global convention}. We will often need to fix a maximal compact subgroup \(K\subseteq G(\A)\), note that the topology is not the Zariski topology but the one specified by \todo[inline]{give the reference. I have it somewhere later too}, this is someetimes known as the "Hausdorff" topology. These maximal compact subgroups are not unique and as such when fixing one it can be arranged to have many nice properties \cite[I.1.4]{moeglinSpectralDecompositionEisenstein1995}. In particular if we have a group \(G\) and a fixed Borel \(B\):
    \begin{itemize}
