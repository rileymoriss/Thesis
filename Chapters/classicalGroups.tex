...
\begin{example}
    ...
    \begin{example}[Bad Etymology]
        \(\GL_1\) is a split torus. Notice that 
        \[\GL_1(\C) = \C^\ast,\]
        is isomorphic as abstract groups to \(U(1)\) which when \(U(1)\) is realised as \(\{z\in \C : |z| = 1\}\) is topologically equivalent to the torus \(\mathbb{T}^1 = \mathbb{S}^1\). Note that it is clear that 
        \[\GL_1(\C) \not\cong \mathbb{T}^2, \]
        as topological groups, as the right hand side is compact whilst the left is not.

        Consider the field extension \(\C/\R\). Then \C has the inner product given by 
        ...
        Now if we complexify we have 
        \[\R[x,y]/(x^2 + y^2 - 1) \tensor_\R \C \cong \]
